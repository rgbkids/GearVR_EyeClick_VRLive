...
namespace VRStandardAssets.Utils
{
    ...
    public class Reticle : MonoBehaviour
    {
        ...
        public void SetPosition ()
        {
			...
			{
				//m_ReticleTransform.position = OVRInput.GetLocalControllerPosition(controller);
				Vector3 ctrlPos = OVRInput.GetLocalControllerPosition(controller);
				//				m_ReticleTransform.position = new Vector3 (ctrlPos.x, ctrlPos.y, m_ReticleTransform.position.z);
								m_ReticleTransform.position = ctrlPos + m_Camera.forward * m_DefaultDistance;
				//m_ReticleTransform.position = new Vector3 (ctrlPos.x, ctrlPos.y, m_Camera.position.z) + m_Camera.forward * m_DefaultDistance;

				//
				//m_ReticleTransform.localScale = OVRInput.G * m_DefaultDistance;

				//
				m_ReticleTransform.localRotation = OVRInput.GetLocalControllerRotation(controller);
