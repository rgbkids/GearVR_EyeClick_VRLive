...
//	private long prevMilliTime = long.MaxValue; // modify suzuki
	private long prevMilliTime; // modify suzuki
	...
	private GameObject text360Degree;
	...
	void Start()
	{
		...
		// modify suzuki
		textLoading = GameObject.Find ("TextLoading");
		textLoading.SetActive (false);
		text360Degree = GameObject.Find ("Text360Degree");
		text360Degree.SetActive (false);

		sphere = GameObject.Find ("Sphere100");
		...
		using (AndroidJavaObject joExDir = jcEnvironment.CallStatic<AndroidJavaObject> ("getExternalStorageDirectory")) {
			// ex) path=/storage/emulated/0/jp.co.cname.app/
//			path = joExDir.Call<string>("toString")+"/com.oculus.oculus360videossdk_naotora/";
			path = joExDir.Call<string>("toString");
		}

		//Debug.Log("masaki path="+path);
		//Debug.Log("masaki m_strFileName="+m_strFileName);
		//Debug.Log("masaki File.Exists(m_strFileName)="+File.Exists(m_strFileName));
		//Debug.Log("masaki Application.streamingAssetsPath="+Application.streamingAssetsPath);
		...
		string fileName = packageName.Replace(".", "_") + "_analytic_log.txt";

		// Log
		//System.IO.File.WriteAllText (path + "/" + fileName, "packageName="+packageName);

		StreamWriter sw;
		FileInfo fi;
		fi = new FileInfo(path + "/" + fileName);
		sw = fi.AppendText();
		sw.WriteLine("[" + System.DateTime.Now + "] " + m_strFileName);
		sw.Flush();
		sw.Close();
	...
	void Update()
	{
		...
		if (currentMilliTime - prevMilliTime > 10 * 1000) 
		{
			Debug.Log ("prevMilliTime=" + prevMilliTime);
		...
		try 
		{
			if ((9 * 1000) < (currentMilliTime - startMilliTime) && (currentMilliTime - startMilliTime) < (12 * 1000)) 
			{
				text360Degree.SetActive (true);
			} 
			else 
			{
				text360Degree.SetActive (false);
		...
		{
			Debug.Log ("Cancel! prevMilliTime=" + prevMilliTime);
		...
		{
			#if UNITY_STANDALONE
			if (OVRInput.GetDown (OVRInput.Button.Back)) // 1回押すと戻らない？挙動おかしいので保留
			{
				Debug.Log ("OVRInput.GetDown(OVRInput.Button.Back)! prevMilliTime=" + prevMilliTime);
	...
	Call_SetUnityTexture (m_VideoTexture.GetNativeTextureID ());
		...
		if (m_CurrentState != Call_GetStatus())
		{
			...
			else if (m_CurrentState == MEDIAPLAYER_STATE.END)
			{
				...
				// modify suzuki
				Debug.Log("MEDIAPLAYER_STATE.END!");
